...
namespace NzbDrone.Core.Music
{
    ...
    {
        void SetAlbumMonitoredStatus(Author artist, MonitoringOptions monitoringOptions);
    }
    ...
    public class AlbumMonitoredService : IAlbumMonitoredService
    {
        ...
        public void SetAlbumMonitoredStatus(Author artist, MonitoringOptions monitoringOptions)
        {
            if (monitoringOptions != null)
            {
                ...
                if (monitoredAlbums.Any())
                {
                    ToggleAlbumsMonitoredState(
                        albums.Where(s => monitoredAlbums.Any(t => t == s.ForeignBookId)), true);
                    ToggleAlbumsMonitoredState(
                        albums.Where(s => monitoredAlbums.Any(t => t != s.ForeignBookId)), false);
                }
                else
                {
        ...
        private void ToggleAlbumsMonitoredState(IEnumerable<Book> albums, bool monitored)
        {
