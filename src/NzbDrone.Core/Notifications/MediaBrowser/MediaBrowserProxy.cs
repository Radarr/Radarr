    ...
    public class MediaBrowserProxy
    {
        ...
        public HashSet<string> GetPaths(MediaBrowserSettings settings, Movie movie)
        {
            ...
            var request = new HttpRequestBuilder(url)
                ...
                .AddQueryParam("includeItemTypes", "Movies")
            ...
            try
            {
                var paths = ProcessGetRequest<MediaBrowserItems>(request, settings).Items
                    .GroupBy(
                        item =>
                        {
                            if (
                                item is { ProviderIds.ImdbId: string imdbid }
                            ...
                            if (
                                item is { ProviderIds.TmdbId: string tmdbid }
                ...
                {
                    _logger.Trace("Could not find movie by name nor id");
        ...
        public void Update(MediaBrowserSettings settings, string moviePath, string updateType)
        {
            ...
            request.SetContent(
                new
                {
                    Updates = new[] { new { Path = moviePath, UpdateType = updateType } }
                }.ToJson());
