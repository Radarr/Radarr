...
namespace NzbDrone.Core.Parser
{
    public class QualityParser
    {
        ...
        private static readonly Regex RemuxRegex = new Regex(@"^(?=.*((\d{4})|720p|UHD))(?=.*\b(Blu[-_. ]?ray|HD[-_. ]?DVD|BD|UHD)?)(?!.*WEB[-_. ]?DL)(?=.*(\[?REMUX\b|((?<=\d{4}).*German.*(DL)?)(?=.*\b(AVC|HEVC|VC[-_. ]?1|MVC|MPEG[-_. ]?2)))).*", RegexOptions.Compiled | RegexOptions.IgnoreCase);
