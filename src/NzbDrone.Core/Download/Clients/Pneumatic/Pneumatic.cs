...
namespace NzbDrone.Core.Download.Clients.Pneumatic
{
    public class Pneumatic : DownloadClientBase<PneumaticSettings>
    {
        ...
        public override async Task<string> Download(RemoteMovie remoteMovie, IIndexer indexer)
        {
            ...
            _logger.Debug("Downloading NZB from: {0} to: {1}", url.SanitizeForLog(), nzbFile.SanitizeForLog());
            await _httpClient.DownloadFileAsync(url, nzbFile);

            _logger.Debug("NZB Download succeeded, saved to: {0}", nzbFile.SanitizeForLog());
